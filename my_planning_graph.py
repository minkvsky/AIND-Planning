...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if self.s_levels[level] >= a_node.prenodes:
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        set1 = set(node_a1.action.effect_add) & set(node_a2.action.effect_rem)
        set2 = set(node_a1.action.effect_rem) & set(node_a2.action.effect_add)
        set3 = set1 | set2
        if set3 != set():
            return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        set1 = set(node_a1.action.precond_pos) & set(node_a2.action.effect_rem)
        set11 = set(node_a1.action.precond_neg) & set(node_a2.action.effect_add)
        set2 = set(node_a2.action.precond_pos) & set(node_a1.action.effect_rem)
        set22 = set(node_a2.action.precond_neg) & set(node_a1.action.effect_add)
        set3 = set1 | set11 | set2 |set22
        if set3 != set():
            return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # set1 = set(node_a1.action.precond_pos) & set(node_a2.action.precond_neg)
        # set2 = set(node_a1.action.precond_neg) & set(node_a2.action.precond_pos)
        # set3 = set1 | set2
        # if set3 != set():
        #     return True
        # return False
        for pre_node1 in node_a1.parents:
            for pre_node2 in node_a2.parents:
                if pre_node1.is_mutex(pre_node2):
                    return True
        return False
        ...
        def level_cost(g):
            for level, literals in enumerate(self.s_levels):
                ...
                for node_s in literals:
                    if node_s.is_pos:
                        literalls.append(expr(node_s.symbol))
                    else:
                        literalls.append(expr('~{}'.format(node_s.symbol)))
                if g in literalls:
                    return level
            return float('inf')
